...
| 30:00-49:00 | 19:00 | Implicit feedback, pointwise/pairwise objectives, BPR, pre-break activities | Worked explanation + activity | Work the BPR calculation. Students discuss defensible sampled negatives, then complete the BPR Bandit Time MCQ. |
...
| --- | ---: | --- | --- |
| **Which negative is defensible?** | 3:00 | One better-supported sampled negative and one residual uncertainty. | Exposure and availability make comparisons more meaningful; they do not make an unclicked item ground-truth dislike. |
| **Bandit Time: What does BPR optimise?** | 3:00 | **B** - increase the clicked item's score relative to the sampled item's score. | BPR optimises score differences, not absolute scores or a blanket dislike label. |
...
| **Bandit Time: Which evidence supports NeuMF?** | 3:00 | **C** - a stable, meaningful gain under the same protocol that fits the service budget. | Capacity is a claim to test, not a default upgrade. |
...
- Test every `w04-` timer: start, pause, resume, expiry, and pause on slide exit.
...
- State the timebox before each activity and limit group reports to preserve the 90-minute plan.
