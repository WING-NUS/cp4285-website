...
subtitle: "CP4285 Modern Recommendation Systems — Week 01"
...
## <span style="color:#ffffff;">👥 Discussion: Which Similarity Axis?</span> {background-color="#003D7C"}

```{=html}
<div style="border:clamp(3px,.6vw,6px) solid #EF7C00;padding:1.1em 1.45em;color:#fff;font-size:.72em;line-height:1.3;">
  <p style="font-size:1.34em;font-weight:bold;color:#EF7C00;margin:0 0 .6em;">When would user-based or item-based similarity be more effective?</p>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.6em;">
    <div style="padding:.8em 1em;border:1px solid rgba(255,255,255,.55);border-radius:8px;">
      <p style="font-size:1.13em;font-weight:bold;margin:0 0 .35em;color:#fff;">User-based: compare rows</p>
      <p style="margin:0;">When might a person's nearest neighbours be reliable enough to recommend from?</p>
    </div>
    <div style="padding:.8em 1em;border:1px solid rgba(255,255,255,.55);border-radius:8px;">
      <p style="font-size:1.13em;font-weight:bold;margin:0 0 .35em;color:#fff;">Item-based: compare columns</p>
      <p style="margin:0;">When might relationships between items be more stable or useful?</p>
    </div>
  </div>
  <div style="margin-top:1em;padding:.75em 1em;background:#fff;color:#293241;border-radius:8px;">
    <strong style="color:#003D7C;">Pairs — 3 minutes:</strong> choose a setting (e.g., films, groceries, music, courses). Make a claim for one axis, then give one data-pattern or product reason that could reverse your choice.<br/><strong style="color:#003D7C;">Suit lens:</strong> which treatment lens would change your choice or your confidence in it?
  </div>
</div>
```

::: notes
Untimed discussion slide. Give pairs roughly three minutes, but do not start a countdown timer. Invite one user-based and one item-based argument. Productive reasons include the number and stability of items, changing user tastes, repeatability of item relationships, sparsity patterns, and computational update costs. Close by asking which card-suit treatment lens would alter the conclusion or its confidence. Make clear that “more effective” depends on the data and use case, not a universal rule.
...
::: notes
Use this key slide to contrast methods rather than declare one universal winner. Pearson and adjusted cosine both centre ratings but on different comparison axes; raw cosine preserves the original scale; significance weighting adjusts confidence in any base similarity.
...
4. rank candidate items by $\hat r_{uj}$
:::
:::
...
## 🔑 Key Point: A Neighbourhood Is a Choice {background-color="#003D7C"}

<div style="color:#fff;font-size:1.05em;line-height:1.35;margin-top:.8em;">
<p><u>Collaborative filtering turns partial overlaps into a local estimate — and every choice of overlap, normalisation, threshold, and top-\(k\) changes that estimate.</u></p>
<p style="margin-top:1.2em;color:#EF7C00;font-weight:bold;">🤞 Before trusting a neighbour, ask: what evidence made it close?</p>
</div>

::: notes
Close the section by making the modelling choices visible: overlap, centring, the reliability discount, and neighbour selection each shape the result. The next section can examine requirements and knowledge-based methods as a different response to missing or insufficient collaborative evidence.
